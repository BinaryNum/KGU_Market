...
public class BCDao {
	...
	public BCDao() {
		...
		try {
			...
			dataSource = (DataSource) context.lookup("java:comp/env/jdbc/mydb");//java:comp/env/jdbc/myoracle
		} catch (Exception e) {
	...
	public void write(String bName, String bTitle, String bContent, String file, int price) {
		...
		try {
			...
			preparedStatement.setString(4, "C:\\LKLKimg\\"+file);
	...
	public ArrayList<BDto> ranking() {
		...
		try {
			...
			String query = "select bId, bName, bTitle, bContent, bDate, bHit, bGroup, bStep, bIndent, img,price, soldFlag "
					+ "from ("
					+ "select * from used_board order by bhit desc"
					+ ") LIMIT 3";
			preparedStatement = connection.prepareStatement(query);
			resultSet = preparedStatement.executeQuery();
